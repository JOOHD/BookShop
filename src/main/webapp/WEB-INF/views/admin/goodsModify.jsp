...
<body>
		...
		<div class="admin_content_main">
			<form action="/admin/goodsModify" method="post" id="modifyForm">
				...
				<div class="form_section">
					...
					<div class="form_section_content">
						<input id="authorName_input"  
							readonly="readonly"
							value="${goodsInfo.authorName}"> 
	...
	$(document).ready(function(){
		...
		$(function(){	
			...
			/*
				수정 페이지이기 때문에 DB에 저장된 데이터가 출력이 되어야 한다. (fix 된 데이터이기 때문에 저장된 데이터 출력)
				문제는 <input> 태그에 value 속성을 추가해도 datepicker가 적용되었기 때문에 출력이 안 된다.
				
				datepicker를 적용시키는 코드에 DB에 저장된 데이터가 출력이 되도록 하는 코드를 따로 추가해주어야 한다.
			*/
		...
		/* 카테고리 */
		// goodsDetail 처럼 DB에 저장된 카테고리가 출력이 되면서, 
		// '상품 등록 페이지'와 같이 대분류 선택, 혹은 중분류 선택에 따라서 하위분류가 초기화 및 변경 되도록 해야 한다.
		// 따라서 '상품 조회', '상품 등록' 페이지에서의 사용한 상품 카테고리 관련 코드를 활용.
		...
		/*
			페이지에 들어가 보면 0으로 되어 있지만,
			실질적으로 서버에 전송되어야 할 할인율 데이터는 type이 hidden인 <input> 태그에 저장되어 있습니다. 
			해당 <input> 태그에는 할인율이 소수로 저장되어 있는데, 자연수로 변경하여 사용자에게 노출되는 <input> 태그 값으로 삽입할 것입니다.. 
			더불어 원가와 할인율을 통해 할인 가격이 얼마인지도 사용자가 볼 수 있도록 해줄 것입니다.
		*/
		
		/* 할인율 인터페이스 출력 */
		let bookPriceInput = $("input[name='bookPrice']");		// 상품 각겨 입력란
		let discountInput = $("input[name='bookDiscount']");	// 상품 할인율 입력란
		
		let bookPrice = bookPriceInput.val();					// 상품 가격
		let rawDiscountRate = discountInput.val();				// 할인율 소수점(뒤에 가공해서 사용자에게는 자연두로 노출)
		let discountRate = rawDiscountRate * 100;				// 할인율
		
		let discountPrice = bookPrice * (1-rawDiscountRate);	// 할인율이 적용된 상품
