							...
							<table>
								...
								<tbody>
									...
									<tr>
										<th>주소</th>
										<td>
											${memberInfo.memberAddr1} (우편번호)<br> 
											${memberInfo.memberAddr2}<br>
				...
				<div class="orderGoods_div">
					...
					<div class="goods_kind_div">
						주문상품 <span class="goods_kind_div_kind"></span>종 
							  <span class="goods_kind_div_count"></span>개
					</div>
					<!-- 상품 테이블 -->
		...
		<div class="footer">
			<div class="footer_container">
				...
				<div class="footer_right">
					...
					<br>
					<br>
					COPYRIGHT(C) <strong>Joo.tistory.com</strong>	ALL RIGHTS RESERVED.
				</div>
				<div class="clearfix"></div>
			</div>
		</div> <!-- class="footer" -->		
...
function showAdress(className) {
	...
	/* 버튼 색상 변경 */
		...
		/* 지정 색상 변경 */
			$(".address_btn_" + className).css('background', '#3c3838');
...
$(".order_point_input").on("propertychange change keyup paste input", function() {
	...
	// 회원이 소유한 포인트는 '문자열'로 인식
	const maxPoint = parseInt('${memberInfo.point}');
	//  ($(this)는 $(".order_point_input")을 의미합니다.)
	let inputValue = parseInt($(this).val());

	if (inputValue < 0) {
		$(this).val(0); // 사용자가 0보다 작은 값 입력 시, 0출력
	} else if (inputValue > maxPoint) {
		$(this).val(maxPoint); // '사용 가능한' 포인트 보다 많은 값 입력 시, maxPoint
	}

	/*
		포인트 변경이 이루어지는 경우
		1.'포인트 입력'란에 직접 입력 - order_point_input
		2.'포인트 버튼'을 클릭한 경우
	 */
	setTotalInfo(); // 회원 포인트 사용에 따른 '주문 종합 정보' 변경, 포인트 버튼 클릭 시 적용.
...
$(".order_point_input_btn").on("click", function() {
	...
	// maxPoint : 사용자가 가지고 있는 모든 포인트
	const maxPoint = parseInt('${memberInfo.point}');
	// "order_point_input_btn" 태그에 심어진 data-state 속성 값을 대입
	...
	if (state == 'N') {
		...
		/* 모두사용 */
		//값 변경
		$(".order_point_input").val(maxPoint); // maxPoint 가져오기
		//글 변경
		...
		/* 사용취소 */
		//값 변경
		$(".order_point_input").val(0); // 사용취소 클릭으로 값 = 0
		//글 변경
	...
	/*
		포인트 변경이 이루어지는 경우
		1.'포인트 입력'란에 직접 입력
		2.'포인트 버튼'을 클릭한 경우 - order_point_input_btn
	 */
	setTotalInfo();
...
$(".order_btn").on("click", function(){
	...
	/* 주소 정보 & 받는이 */
	$(".addressInfo_input_div").each(function(i, obj){
		if($(obj).find(".selectAddress").val() === 'T'){ // db에서 가져온 주소
